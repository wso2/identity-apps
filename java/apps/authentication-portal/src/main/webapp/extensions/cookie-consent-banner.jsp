...
<%
    ...
    if (is_cookiepro_enabled == null) {
        is_cookiepro_enabled = "false";
    }
    ...
    if (StringUtils.equals(tenantForTheming, IdentityManagementEndpointConstants.SUPER_TENANT)) {
        cookiePolicyURL = "https://wso2.com/cookie-policy";
    }
%>

<%
    if (!StringUtils.isBlank(cookiePolicyURL) && (Boolean.FALSE.toString()).equals(is_cookiepro_enabled)) {
%>
<div id="cookie-consent-banner" data-testid="cookie-consent-banner" class="ui segment cookie-consent-banner inverted aligned-right hidden transition">
    ...
    <div class="banner-content" data-testid="cookie-consent-banner-content">
        <div class="title" data-testid="cookie-consent-banner-content-title">
            <%=AuthenticationEndpointUtil.i18n(resourceBundle, "cookie.consent.banner.content")%>
            <a href="<%= StringEscapeUtils.escapeHtml4(cookiePolicyURL) %>" target="_blank" rel="noopener noreferrer" data-testid="cookie-policy-link">
                <%=AuthenticationEndpointUtil.i18n(resourceBundle, "cookie.policy")%>
            </a>.
        </div>
    </div>
    <div class="actions" data-testid="cookie-consent-banner-actions">
        <button data-testid="cookie-consent-banner-confirm-button" class="ui fluid primary button" type="button" onclick="onCookieConsentClear(this)">
            <%=AuthenticationEndpointUtil.i18n(resourceBundle, "cookie.consent.banner.clear")%>
        </button>
    </div>
</div>
<% } %>
<%
    if (!StringUtils.isBlank(cookiePolicyURL) && (Boolean.TRUE.toString()).equals(is_cookiepro_enabled)) {
%>
    ...
    <!-- CookiePro Cookies Consent Notice end for asgardeo.io -->
<% } %>
