...
/**
 * Resolve the i18n meta hash from the built i18n module bundle.
 *
 * @returns Resolved meta hash.
 */
const resolveMetaHash = (): string => {
    const i18nBundleDir: string = path.resolve(__dirname, "../../modules/i18n/dist/bundle");

    if (!fs.existsSync(i18nBundleDir)) {
        return "";
    }

    const metaFile: string | undefined = fs
        .readdirSync(i18nBundleDir)
...
const myAccountDevHtmlPlugin = (basePath: string): PluginOption => {
    const normalizedBasePath: string = basePath.endsWith("/") ? basePath : `${basePath}/`;
    const basePathWithoutTrailingSlash: string = normalizedBasePath.replace(/\/$/, "");
    const appSegment: string = basePathWithoutTrailingSlash.split("/").filter(Boolean).pop() || "myaccount";
...
<html>
  <head>
    ...
    ${themeStyleSheetPath
        ? `<link id="ltr-stylesheet" href="${themeStyleSheetPath}" rel="stylesheet" type="text/css" />`
        : ""}
    ...
    <script>
      ...
      var isAdaptiveAuthenticationAvailable;
      var isOrganizationManagementEnabled;
    </script>
    ...
    return {
        ...
        configureServer(server: ViteDevServer) {
            server.middlewares.use((
                ...
                const existingPath: string | undefined = [ resolvedBundlePath ].find((candidatePath: string) => {
    ...
    return {
        ...
        configureServer(server: ViteDevServer) {
            server.middlewares.use((
                ...
                const existingPath: string | undefined = [ resolvedThemeLibPath ].find((candidatePath: string) => {
                    return fs.existsSync(candidatePath) && fs.statSync(candidatePath).isFile();
                });
...
export default defineConfig(({ mode }: { mode: string }) => {
    ...
    return {
        ...
        plugins: [
            ...
            basicSsl(),
            myAccountDevHtmlPlugin(buildMode.publicBase),
