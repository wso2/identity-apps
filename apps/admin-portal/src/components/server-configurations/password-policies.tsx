...
import { PasswordPoliciesInterface } from "../../models";
...
export const PasswordPolicies: FunctionComponent<PasswordPoliciesProps> = (
    ...
    const handleUpdateError = (error) => {
        if (error.response && error.response.data && error.response.data.detail) {
            dispatch(addAlert({
                description: t("adminPortal:components.serverConfigs.passwordPolicies.notifications." +
                    "updateConfigurations.error.description", { description: error.response.data.description }),
                level: AlertLevels.ERROR,
                message: t("adminPortal:components.serverConfigs.passwordPolicies.notifications.updateConfigurations." +
                    "error.message")
            }));
        } else {
    ...
    const handleRetrievalError = (error) => {
        if (error.response && error.response.data && error.response.data.detail) {
            dispatch(addAlert({
                ...
                level: AlertLevels.ERROR,
                message: t("adminPortal:components.serverConfigs.passwordPolicies.notifications." +
    ...
    const savePasswordPoliciesConfigs = (key, value) => {
        ...
        switch (key) {
            case ServerConfigurationsConstants.PASSWORD_HISTORY_ENABLE:
                successNotification.description = t("adminPortal:components.serverConfigs.passwordPolicies." +
        ...
        <Grid className="middle aligned mt-1">
            <Grid.Row columns={ 2 } className="inner-list-item mt-3 mb-3">
                ...
                <Grid.Column mobile={ 15 } tablet={ 15 } computer={ 15 }>
                    <Field
                        label={ t("adminPortal:components.serverConfigs.passwordPolicies.passwordHistory." +
                            "form.passwordHistoryCount.label") }
                        name={ ServerConfigurationsConstants.PASSWORD_HISTORY_COUNT }
                        placeholder={ t("adminPortal:components.serverConfigs.passwordPolicies." +
                            "passwordHistory.form.passwordHistoryCount.placeholder") }
                        required={ true }
                        requiredErrorMessage={ t("adminPortal:components.serverConfigs.passwordPolicies." +
                    ...
                    <Hint disabled={ !passwordPoliciesConfigs.passwordHistoryEnable }>
                        { t("adminPortal:components.serverConfigs.passwordPolicies.passwordHistory." +
                ...
                <Grid className="middle aligned mt-1">
                    <Grid.Row columns={ 3 } className="inner-list-item">
                        <Grid.Column className="first-column pl-3" mobile={ 14 } tablet={ 14 } computer={ 14 } >
                            { t("adminPortal:components.serverConfigs.passwordPolicies." +
    ...
    const showPasswordPolicyOtherSettings: ReactElement = (
        <Grid className="middle aligned mt-1">
            <Grid.Row columns={ 2 } className="inner-list-item mt-3 mb-3">
                ...
                <Grid.Column mobile={ 15 } tablet={ 15 } computer={ 15 }>
                    <Field
                        label={ t("adminPortal:components.serverConfigs.passwordPolicies.passwordPatterns." +
                    ...
                    <Hint disabled={ !passwordPoliciesConfigs.passwordPolicyEnable }>
                        { t("adminPortal:components.serverConfigs.passwordPolicies.passwordPatterns." +
    ...
    return (
        <Section
            description={ t("adminPortal:components.serverConfigs.passwordPolicies.description") }
            header={ t("adminPortal:components.serverConfigs.passwordPolicies.heading") }
