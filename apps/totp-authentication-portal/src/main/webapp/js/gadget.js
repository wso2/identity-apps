...
function drawPage() {
	...
	var start =
		...
		'                    <div class="controls">' +
		'                        <input type="text" id="profileName" disabled="" name="profileName" placeholder="Profile Name" value="' +
		json.return.profileName +
		'" class="required"/>' +
		"                    </div>" +
		"                </div>";
	...
	for (var i in json.return.fieldValues) {
		if (
			json.return.fieldValues[i].displayName == "Encoding" ||
			json.return.fieldValues[i].displayName == "Secret Key"
		) {
			continue;
		}
		body =
			...
			json.return.fieldValues[i].displayName;
		...
		if (json.return.fieldValues[i].displayName == "Refresh Secret Key") {
			if (json.return.fieldValues[i].fieldValue != "") {
				body =
					body +
					'<input type="checkbox" checked name="refreshenable" onclick="validateRefreshSecret();"/></div>\n<br>';
				continue;
			} else {
				body =
					body +
					' <input type="checkbox" name="refreshenable" onclick="validateRefreshSecret();"/></div>\n<br>';
				continue;
			}
		}
		...
		if (json.return.fieldValues[i].displayName != "Enable TOTP") {
			if (json.return.fieldValues[i].readOnly == "true") {
				body =
					...
					json.return.fieldValues[i].fieldValue +
					'" id="' +
					json.return.fieldValues[i].claimUri +
					'" name="' +
					json.return.fieldValues[i].claimUri +
					'"  />\n' +
			...
			if (encoding != "Invalid") {
				if (json.return.fieldValues[i].fieldValue != "") {
					body +=
						' <input type="checkbox" checked name="totpenable" onclick="validateCheckBox();"/>\n<br><br>' +
						' <div id="qrdiv">' +
						' <form name="qrinp">' +
					...
					body +=
						'<input type="checkbox" name="totpenable" onclick="validateCheckBox();" style="float:left"/>' +
				...
				body +=
					'<input type="checkbox" name="totpenable" onclick="validateCheckBox();" style="float:left"/>' +
					'<label id="tokenInvalid" style="margin-left:20px">Invalid Token Please Reconfigure</label>' +
					'<img id="totpQRCode" src="' +
					json.return.fieldValues[i].fieldValue +
					'" style="Display:none">' +
					'<canvas id="qrcanv">';
					...
					if (resp.success == true) {
						...
						if (typeof resp.reLogin != "undefined" && resp.reLogin == true) {
							window.top.location.href =
								window.location.protocol + "//" + serverUrl + "/dashboard/logout.jag";
						} else {
				...
				if (isArray(deviceMetadata)) {
					for (var i in deviceMetadata) {
						middle =
							...
							"</td>\n" +
							"                    <td class='txtAlnCen'>\n" +
							'                        <a title="" onclick="removeFIDO(\'' +
							deviceMetadata[i] +
							'\');" href="javascript:void(0)"><i class="icon-trash"></i> Remove</a>\n' +
							"                    </td>\n" +
							"                </tr>\n";
					}
				} else {
