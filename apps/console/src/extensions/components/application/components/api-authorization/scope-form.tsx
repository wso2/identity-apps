...
export const ScopeForm: FunctionComponent<ScopeFormInterface> = (
    ...
    const [ previousSelectedScopes, setPreviousSelectedScopes ]
        = useState<AuthorizedPermissionListItemInterface[]>(subscribedAPIResource.permissions);
    ...
    useEffect(() => {
        ...
        subscribedAPIResource.allPermissions?.forEach((scope: AuthorizedPermissionListItemInterface) => {
            options.push({
                content: (
                    <Header
                        as="h6"
    ...
    useEffect(() => {
        ...
        subscribedAPIResource.permissions.forEach((scope: AuthorizedPermissionListItemInterface) => {
     ...
     */
    useEffect(() => {
        setIsSelectAllDisabled(subscribedAPIResource.allPermissions?.length === selectedScopes?.length);
    ...
    const updateScopesOfAPIResource = (): void => {
        ...
        setIsLoading(true);
        patchScopesOfAPIResource(appId, subscribedAPIResource?.apiId, addedScopesIdentifiers, removedScopesIdentifiers)
            .then(() => {
                ...
                const addedScopes: AuthorizedPermissionListItemInterface[]
                    = subscribedAPIResource.allPermissions?.map((scope: AuthorizedPermissionListItemInterface) => {
            ...
            .catch((error: IdentityAppsApiException) => {
                switch(error?.code) {
                    case APIResourcesConstants.UNAUTHORIZED_ACCESS:
                        dispatch(addAlert<AlertInterface>({
                            description: t("extensions:develop.applications.edit.sections.apiAuthorization" +
                                ".sections.apiSubscriptions.notifications.patchScopes.unauthorizedError.description" ),
                            level: AlertLevels.ERROR,
                            message: t("extensions:develop.applications.edit.sections.apiAuthorization" +
                                ".sections.apiSubscriptions.notifications.patchScopes.unauthorizedError.message" )
                        }));
    ...
    const handleDropdownChange = (event: SyntheticEvent<HTMLElement, Event>, data: DropdownProps): void => {
        ...
        if (scopes.length >= previousSelectedScopes.length) {
            const scopeObject: AuthorizedPermissionListItemInterface = subscribedAPIResource.allPermissions?.find(
    ...
    const resetDropdown = (): void => {
        ...
        setPreviousSelectedScopes(subscribedAPIResource.permissions);
        setAddedScopesIdentifiers([]);
        setRemovedScopesIdentifiers([]);
        setIsUpdateButtonDisabled(true);
    };
                                                                    ...
                                                                    <Button
                                                                        ...
                                                                        onClick={ () =>
                                                                            handleBulkDropdownChange(
                                                                                subscribedAPIResource.allPermissions
                                                                                    ? subscribedAPIResource
                                                                                        .allPermissions
                                                                                    : [],
                                                                                false ) }
