...
export const GroupUsersList: FunctionComponent<GroupUsersListProps> = (props: GroupUsersListProps): ReactElement => {
    ...
    const handleAssignedItemCheckboxChange = (user: UserBasicInterface) => {
        ...
        if (checkedUsers.some((checkedUser: UserBasicInterface) => checkedUser.id===user.id)) {
            checkedUsers.splice(checkedUsers.findIndex(
                (checkedUser: UserBasicInterface) => checkedUser.id===user.id), 1);
                            ...
                            addModalUserList?.map((user: UserBasicInterface, index: number) => {
                                ...
                                return (
                                    <TransferListItem
                                        ...
                                        isItemChecked={
                                            newlySelectedUsers && 
                                                newlySelectedUsers.some(
                                                    (newUser: UserBasicInterface) => newUser.id===user.id)
                                        }
