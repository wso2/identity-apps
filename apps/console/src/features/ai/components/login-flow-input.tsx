...
const LoginFLowAIComponent: React.FC<LoginFlowAIComponentProps> = ({onGenerateClick, onGenerate, aiBannerState, onComplete}): ReactElement => {
    ...
    // //State to keep the user claims.
    // const [userClaims, setUserClaims] = useState<Claim[]>(null);

    // //State to keep the user claims URIs.
    // const [claimURIs, setClaimURIs] = useState<ClaimURIs[]>([]);

    
    // //Fucntion to fetch the user claims.
    ...
    //         .then((response: Claim[]) => {
    //             console.log("User claims fetched response", response);
    ...
    //         })
    //         .finally(() => {
    //             console.log("User claims fetched", userClaims);
    //         });
    // };
    ...
    const handleGenerateButtonClick = async(e) => {
        ...
        //Fetching required userClaims
        // fetchUserClaims()
        // console.log("User claims fetched:", userClaims);
        // Call the API to generate the branding.
        try{
            const response: any = await axios.post(
                "http://localhost:3000/loginflow/generate",
                {   
                    user_query: loginFlowInput,
                    // Call the endpoints and fetch the requirde user data
                    user_claims:  { "failedLoginAttempts": "http://wso2.org/claims/identity/failedLoginAttemptsBeforeSuccess",
                                    "magicLinkExpiray": " http://wso2.org/claims/MgcLinkEx"},
                ...
                // add user store, tentant details to the input.
                {
                    headers:{
                        "Trace-Id": traceId
                    },
