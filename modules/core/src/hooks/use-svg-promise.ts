 ...
 */
export const useSVGPromise = (
    imagePromise: Promise<SVGRLoadedInterface> | SVGRLoadedInterface
): [ string, FunctionComponent<SVGProps<SVGSVGElement>> ] => {

    const [ image, setImage ] = useState(imagePromise);
    const [ imageAsDataURL, setImageAsDataURL ] = useState<string>(null);
    const [
        imageAsReactComponent,
        setImageAsReactComponent
    ] = useState<FunctionComponent<SVGProps<SVGSVGElement>>>(null);

    useEffect(() => {
        if (image instanceof Promise) {
            image
                .then((response: SVGRLoadedInterface) => {
                    setImageAsDataURL(response?.default);
                    setImageAsReactComponent(response?.ReactComponent);
                });
