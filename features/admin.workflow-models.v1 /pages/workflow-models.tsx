...
const WorkflowDefinitions: FunctionComponent<WorkflowsPageInterface> = (
    ...
    const searchApprovalProcessList = (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (changeValue.length > 0) {
            const searchResult: WorkflowListItemInterface[] = approvalProcesses.filter(
                (item: WorkflowListItemInterface) => item.name.toLowerCase().indexOf(changeValue.toLowerCase()) !== -1
            );
    ...
    const handleOnDelete = (approvalProcess: WorkflowListItemInterface): void => {
        deleteWorkflowById(approvalProcess.id)
            .then(() => {
                handleAlerts({
    ...
    return (
        <PageLayout
            action={
                !isApprovalProcessListRequestLoading &&
