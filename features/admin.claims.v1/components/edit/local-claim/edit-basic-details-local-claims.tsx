...
export const EditBasicDetailsLocalClaims: FunctionComponent<EditBasicDetailsLocalClaimsPropsInterface> = (
    ...
    const dataTypeOptions: DropDownItemInterface[] = [
        {
            text: t("claims:local.forms.dataType.options.string"),
        ...
        {
            text: t("claims:local.forms.dataType.options.date_time"),
            value: DataType.DATE_TIME
        },
        {
            text: t("claims:local.forms.dataType.options.complex"),
    ...
    useEffect(() => {
        if (claim.canonicalValues && Array.isArray(claim.canonicalValues)) {
            setCanonicalValues(claim.canonicalValues);
        }
    ...
    const onSubmit = (values: Record<string, unknown>) => {
        ...
        if (isDistinctAttributeProfilesDisabled) {
            ...
            data = {
                ...
                canonicalValues: values?.canonicalValue !== undefined
                    ? values.canonicalValues as string[]
                    : canonicalValues,
                claimURI: claim.claimURI,
                dataType: values?.dataType !== undefined
                    ? values.dataType?.toString()
                    : dataType,
            ...
            <EmphasizedSegment padded="very">
                ...
                >
                    ...
                    <Field.Dropdown
                        ...
                        value={ dataType }
                        onChange={ (event, data) => {
                            if (data.value === DataType.COMPLEX) {
                                setSubAttributes([]);
                            }
                    ...
                    { dataType === DataType.STRING && (
                        <>
                            <div style={ { display: "flex", alignItems: "center" } }>
                                <Field.Input
                                    ariaLabel="canonical-values-input"
                                    name="canonicalValuesInput"
                                    label={ t("claims:local.forms.canonicalValues.label") }
                                    placeholder={ t("claims:local.forms.canonicalValues.placeholder") }
                                    onChange={ (event) => {
                                        setCanonicalValue(event.target.value); // Update canonicalValue state
                                    } }
                                    value={ canonicalValue } // Bind the input to canonicalValue
                                    data-testid={ `${testId}-form-canonical-values-input` }
                                    style={ { flex: 1 } } // Make the input take available space
                                    inputType="default" // Specify the input type
                                    maxLength={ 255 } // Set the maximum length
                                    minLength={ 1 } // Set the minimum length
                                />
                                <IconButton
                                    disabled={ !canonicalValue || isReadOnly }
                                    onClick={ () => {
                                        if (canonicalValue && !canonicalValues.includes(canonicalValue)) {
                                            setCanonicalValues([ ...canonicalValues, canonicalValue ]);
                                            setCanonicalValue(""); // Clear the input field after adding
                                        }
                                    } }
                                    data-componentid={ `${testId}-add-canonical-value-icon` }
                            ...
                            <div>
                                { canonicalValues.map((value, index) => (
                                    <div
                                        style={ { alignItems: "center", display: "flex",
                                            justifyContent: "space-between" } }
                                        key={ index }
                                    >
                                        ...
                                        <IconButton
                                            disabled={ isReadOnly }
                                            onClick={ () => {
                                                setCanonicalValues(canonicalValues.filter((item) => item !== value));
                                            } }
                                            data-componentid={ `${testId}-delete-canonical-value-${index}` }
                                            style={ { marginLeft: "auto" } }
                                        >
                                            <TrashIcon />
                                        </IconButton>
