...
export const AddOrganizationModal: FunctionComponent<AddOrganizationModalPropsInterface> = (
    ...
    const [ isInvalidOrgHandle, setIsInvalidOrgHandle ] = useState<boolean>(false);

    const orgHandleRegex: string = "^[a-zA-Z0-9 .\\-_]+$";
    ...
    const updateOrgHandle = (orgName: string): void => {

        if (isEmpty(orgName)) {
            setOrgHandle(undefined);
        } else {
            const sanitizedValue: string = orgName.trim().toLowerCase().replace(/^[^a-z]*|[^a-z0-9]/g, "");

            setAutoGeneratedOrgName(sanitizedValue);
        }
    };
     ...
     */
    function setAutoGeneratedOrgName(sanitizedValue: string) {
    ...
    const checkOrgHandleValidity: any = debounce(async (orgHandle: string, t: any) => {
        if (!orgHandle) {
            return;
        }
        setIsInvalidOrgHandle(true);
        ...
        // Check availability.
            ...
            const regex: RegExp = new RegExp(orgHandleRegex);
                            ...
                            <Grid.Column width={ 16 }>
                                ...
                                >
                                    <Field.Input
                                        ...
                                        width={ 16 }
                                        listen={ (value: string) => updateOrgHandle(value) }
                                    />
                                    <Field.Input
                                        ...
                                        validate={ (value: string) => checkOrgHandleValidity(value, t) }
                                    />
                                    <Field.Input
