...
import { WebhookResponseInterface, WebhookUpdateRequestInterface } from "../models/webhooks";
 ...
 * @param webhookData - Webhook update request payload.
 * @returns Promise containing the response.
 * @throws Throws an IdentityAppsApiException if the request fails.
 */
const updateWebhook = (
    webhookId: string,
    webhookData: WebhookUpdateRequestInterface
        ...
        .then((response: AxiosResponse) => {
            if (response.status !== 200) {
                throw new IdentityAppsApiException(
                    WebhooksConstants.ERROR_MESSAGES.UPDATE_WEBHOOK_INVALID_STATUS_CODE,
                    null,
                    response.status,
                    response.request,
                    response,
