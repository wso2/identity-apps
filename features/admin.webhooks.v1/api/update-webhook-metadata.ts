...
import {
    ...
    WebhookMetadataUpdateResponseInterface
        ...
        .then((response: AxiosResponse) => {
            if (response.status !== 200) {
                throw new IdentityAppsApiException(
                    WebhooksConstants.ERROR_MESSAGES.UPDATE_WEBHOOK_METADATA_INVALID_STATUS_CODE,
                    null,
                    response.status,
                    response.request,
                    response,
...
export default updateWebhookMetadata;
