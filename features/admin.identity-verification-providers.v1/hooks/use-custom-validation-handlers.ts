...
import useIdVPNameValidation from "./use-idvp-name-validation";
import { IdVPTemplateValidationHandlers } from "../models/dynamic-fields";
...
const useValidationHandlers = (): { customValidations: CustomValidationsFunction } => {
    ...
    const { validateName } = useIdVPNameValidation();
    ...
    const customValidations = async (
        ...
        switch (handler?.name) {
            case IdVPTemplateValidationHandlers.IdVP_NAME:
                validationResult = await validateName(
