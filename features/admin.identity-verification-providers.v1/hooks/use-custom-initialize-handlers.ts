...
import useUniqueIdVPName from "./use-unique-idvp-name";
import { IdVPTemplateInitializeHandlers } from "../models/dynamic-fields";
...
const useInitializeHandlers = (): { customInitializers: CustomInitializeFunction } => {
    ...
    const { generateUniqueName } = useUniqueIdVPName();
    ...
    const customInitializers = async (
        ...
        switch (handler?.name) {
            case IdVPTemplateInitializeHandlers.UNIQUE_IdVP_NAME:
