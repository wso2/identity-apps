...
export class Config {
    ...
    public static resolveServerHost(enforceOrgPath?: boolean, skipAuthzRuntimePath?: boolean): string {
        // If tenant qualified URLs are disabled, tenant path should not be added.
        if (!this.isTenantQualifiedURLsEnabled()) {
            this.getDeploymentConfig().serverOrigin;
        }
    ...
    /**
     * Resolves server host from deployment config.
     *
     * @returns server host.
     */
    private static resolveServerHostFromConfig() {
