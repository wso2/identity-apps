 ...
 *
 * @param payload - Share with all organizations payload.
 * @returns A promise resolving to the API response.
 */
export const shareIdPWithAllOrganizations = (
    payload: ShareIdPWithAllOrganizationsPayloadInterface
        ...
        .then((response: AxiosResponse) => {
            if (response.status !== 200 && response.status !== 202) {
                throw new IdentityAppsApiException(
                    "Failed to share the identity provider with all organizations.",
                    null,
                    response.status,
                    response.request,
                    response,
