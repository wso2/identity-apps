...
import { RoleShareType, ShareType, ShareTypeSwitchApproach } from "@wso2is/admin.users.v1/constants/user-roles";
...
import useGetAgentShare from "../../api/use-get-agent-share";
import {
    AgentSharingPolicy,
...
interface AgentShareFormPropsInterface
...
export const ShareAgentForm: FunctionComponent<AgentShareFormPropsInterface> = (
    ...
    const agentRolesList: RolesV2Interface[] = useMemo(() => {
        if (originalAgentRoles?.Resources && originalAgentRoles.Resources.length > 0) {
    ...
    useEffect(() => {
        ...
        if (orgSharingPolicy === AgentSharingPolicy.ALL_EXISTING_AND_FUTURE_ORGS) {
            ...
            const roleSharingMode: string | undefined = agentShareData?.sharingMode?.roleAssignment?.mode;
    ...
    const resetStates = (shouldMutate: boolean = true): void => {
        ...
        if (shouldMutate) {
            mutateAgentShareDataFetchRequest();
        }
    };
                    ...
                    return {
                        ...
                        roleAssignment: {
                            mode: shareAllRoles
                                ? RoleSharingModes.SELECTED
                                : RoleSharingModes.NONE,
                            roles: shareAllRoles ? (agentRolesList?.map((role: RolesV2Interface) => ({
                                audience: {
    ...
    const shareIndividualRolesWithSelectedOrgs = (): void => {
        ...
        const addOperations: ShareOrganizationsAndRolesPatchOperationInterface[] = Object.entries(tempAddedRoles)
            .map(([ orgId, roles ]: [string, RoleSharingInterface[]]) => {
                ...
                item: ShareOrganizationsAndRolesPatchOperationInterface | null
            ): item is ShareOrganizationsAndRolesPatchOperationInterface => item !== null);
    ...
    const switchShareTypeFromAllToSelected = async (): Promise<void> => {
        if (shareTypeSwitchApproach === ShareTypeSwitchApproach.WITH_UNSHARE) {
            ...
            const sharedOrgIds: string[] =
                (agentShareData?.organizations?.length ?? 0) > 0
                    ? (agentShareData?.organizations ?? []).map((org: SharedOrganizationInterface) => org.orgId)
