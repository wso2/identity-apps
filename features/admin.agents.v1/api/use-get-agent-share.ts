...
import { AgentSharedOrganizationsResponse } from "../models/endpoints";

/**
 * Hook to get agent shared organizations.
 *
 ...
 * @returns SWR response object containing agent shared organizations data.
 */
const useGetAgentShare = (
    agentId: string,
    ...
    const url: string = `${resourceEndpoints.agentSharing}/${agentId}/share${queryString ? `?${queryString}` : ""}`;
    ...
    const { data, error, isLoading, isValidating, mutate } =
        useRequest<AgentSharedOrganizationsResponse, RequestErrorInterface>(
            requestConfig,
            {
...
export default useGetAgentShare;
