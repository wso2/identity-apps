...
/**
 * Create new app-specific SMS template.
 *
 * @param templateType - Template type/id to create.
 * @param appId - Application ID.
 * @param smsTemplate - New SMS template.
 *
 * @returns Created app-specific SMS Template.
 */
const createAppSmsTemplate = (
    templateType: string,
    appId: string,
        ...
        .then((response: AxiosResponse) => {
            if (response.status !== 200 && response.status !== 201) {
                throw new IdentityAppsApiException(
                    "Error occurred while creating the app-specific SMS template.",
                    null,
                    response.status,
                    response.request,
                    response,
...
export default createAppSmsTemplate;
