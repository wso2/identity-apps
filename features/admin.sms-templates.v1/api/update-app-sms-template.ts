...
/**
 * Update the app-specific SMS template.
 *
 ...
 *
 * @returns Updated app-specific SMS template.
 */
const updateAppSmsTemplate = (
    templateType: string,
    appId: string,
        ...
        .then((response: AxiosResponse) => {
            if (response.status !== 200 && response.status !== 201) {
                throw new IdentityAppsApiException(
                    "Error occurred while updating the app-specific SMS template.",
                    null,
                    response.status,
                    response.request,
                    response,
...
export default updateAppSmsTemplate;
