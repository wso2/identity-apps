...
import AdaptiveAuthTemplateInfoModal, {
    AdaptiveAuthTemplateInfoModalPropsInterface
} from "../adaptive-auth-template-info-modal";

describe("AdaptiveAuthTemplateInfoModal", () => {
    /* eslint-disable max-len, sort-keys */
    const defaultProps: AdaptiveAuthTemplateInfoModalPropsInterface = {
        open: true,
        onClose: jest.fn(),
        template: {
            summary:
    ...
    it("renders the AdaptiveAuthTemplateInfoModal component", () => {
        render(<AdaptiveAuthTemplateInfoModal { ...defaultProps } />, { allowedScopes: fullPermissions });

        const adaptiveAuthTemplateInfoModal: Element = screen.getByTestId(
            "adaptive-auth-template-info-modal"
        );

        expect(adaptiveAuthTemplateInfoModal).toBeInTheDocument();
    });
});
