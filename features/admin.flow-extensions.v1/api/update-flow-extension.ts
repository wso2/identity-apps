...
/**
 * Update a Flow Extension connection. All fields are optional; omitted fields are left unchanged
 * by the backend (PATCH semantics).
 *
 * @param extensionId - ID of the Flow Extension to update.
 * @param body - Flow Extension update request body.
 * @returns A promise that resolves with the updated Flow Extension.
        ...
        .then((response: AxiosResponse) => {
            if (response.status !== 200) {
                throw new IdentityAppsApiException(
                    "Failed to update the flow extension.",
                    null,
                    response.status,
                    response.request,
                    response,
