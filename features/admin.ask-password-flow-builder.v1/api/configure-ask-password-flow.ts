 ...
 *
 */
const configureAskPasswordFlow = (payload: Payload): Promise<AxiosResponse> => {
    const requestConfig: RequestConfigInterface = {
        data: payload,
        method: HttpMethods.PUT,
        url: store.getState().config.endpoints.askPasswordFlow
    };
    ...
    return httpClient(requestConfig)
        .then((response: AxiosResponse) => {
            if (response.status !== 200) {
                throw new IdentityAppsApiException(
                    AskPasswordFlowBuilderConstants.FLOW_CONFIG_INVALID_STATUS_ERROR,
                    null,
                    response.status,
                    response.request,
                    response,
        ...
        .catch((error: AxiosError) => {
            throw new IdentityAppsApiException(
                AskPasswordFlowBuilderConstants.FLOW_CONFIG_UPDATE_ERROR,
                error.stack,
                error.code,
                error.request,
                error.response,
...
export default configureAskPasswordFlow;
