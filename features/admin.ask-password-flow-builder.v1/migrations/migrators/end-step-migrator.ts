 ...
 *
 * @param flow - The ask password flow to check
 * @returns True if the flow contains string "End" references
 */
const hasStringEndReference = (flow: AskPasswordFlow): boolean => {
...
/**
 * Migrates old ask password flows with string "End" references to the new END node format.
 *
 * This migrator:
 ...
 *
 * @param flow - The ask password flow to migrate
 * @returns The migrated ask password flow with END nodes, or the original flow if no migration is needed
 */
const endStepMigrator = (flow: AskPasswordFlow): AskPasswordFlow => {
    if (!flow || !flow.steps || !Array.isArray(flow.steps)) {
        return flow;
    }
