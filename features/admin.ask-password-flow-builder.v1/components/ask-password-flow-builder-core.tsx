...
import AskPasswordFlowAILoader from "./ai-ask-password-flow-generation-loader";
import StaticNodeFactory from "./resources/steps/static-step-factory";
import StepFactory from "./resources/steps/step-factory";
import useAIAskPasswordFlowGenerationResult from "../api/use-ai-ask-password-flow-result";
import useGetAskPasswordFlow from "../api/use-get-ask-password-flow";
import useGetAskPasswordFlowBuilderResources from "../api/use-get-ask-password-flow-builder-resources";
import {
    ASK_PASSWORD_FLOW_AI_PROMPT_HISTORY_PREFERENCE_KEY
} from "../constants/ask-password-flow-ai-constants";
import AskPasswordFlowExecutorConstants from "../constants/ask-password-flow-executor-constants";
import useAIGeneratedAskPasswordFlow from "../hooks/use-ai-generated-ask-password-flow";
import useGenerateAskPasswordFlow, {
    UseGenerateAskPasswordFlowFunction
} from "../hooks/use-generate-ask-password-flow";
 ...
 */
const AskPasswordFlowBuilderCore: FunctionComponent<AskPasswordFlowBuilderCorePropsInterface> = ({
    "data-componentid": componentId = "ask-password-flow-builder-core",
    ...rest
}: AskPasswordFlowBuilderCorePropsInterface): ReactElement => {
    ...
    const {
        data: flowData,
        error: flowError
    } = useAIAskPasswordFlowGenerationResult(operationId, flowGenerationCompleted);

    const generateAIAskPasswordFlow: UseGenerateAskPasswordFlowFunction = useGenerateAskPasswordFlow();
    ...
    } = useGetAskPasswordFlow();

    const { data: resources } = useGetAskPasswordFlowBuilderResources();
     ...
     */
    useEffect(() => {
        if (askPasswordFlowFetchRequestError) {
            dispatch(
                addAlert({
    ...
    }, [ askPasswordFlowFetchRequestError ]);

    const handleAIFlowGeneration = async () => {
        const traceID: string = uuidv4();

        await generateAIAskPasswordFlow(userPrompt, traceID);
    ...
    const handleAIGeneratedFlow = (data: { steps: any; }) => {
        ...
        const aiGeneratedAskPasswordFlow: any = {
            "category": "STARTER",
            "config": {
                "data": data
            },
        ...
        setAIGeneratedFlow(aiGeneratedAskPasswordFlow);
        setShowAIGenerationModal(false);
        setIsFlowGenerating(false);
    ...
    useLayoutEffect(() => {
        ...
        askPasswordFlow?.steps,
        isAskPasswordFlowFetchRequestLoading,
        isAskPasswordFlowFetchRequestValidating
    ]);
            ...
            />
            {
                showAIGenerationModal && (
                    <AIGenerationModal
                        ...
                        showHistory={ true }
                        promptHistoryPreferenceKey={ ASK_PASSWORD_FLOW_AI_PROMPT_HISTORY_PREFERENCE_KEY }
                    />
                )
            }
            { isFlowGenerating && <AskPasswordFlowAILoader /> }
        </>
    );
};

export default AskPasswordFlowBuilderCore;
