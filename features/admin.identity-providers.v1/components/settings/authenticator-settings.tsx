...
export const AuthenticatorSettings: FunctionComponent<IdentityProviderSettingsPropsInterface> = (
    ...
    const availableFederatedAuthenticators: FederatedAuthenticatorListItemInterface[] =  useSelector(
    ...
    useEffect(() => {
        ...
        fetchAuthenticators()
            .then((res: FederatedAuthenticatorWithMetaInterface[]) => {
    ...
    const filterTemplates = (): void => {
        ...
        const availableAuthenticatorIDs: string[] = availableAuthenticators?.map(
            (authenticator: FederatedAuthenticatorWithMetaInterface) => authenticator.id);

        const filteredTemplates: IdentityProviderTemplateItemInterface[] = identityProviderTemplates
            ?.filter((template: IdentityProviderTemplateItemInterface) =>
        ...
        filteredTemplates?.sort((a: IdentityProviderTemplateItemInterface, b: IdentityProviderTemplateItemInterface) =>
    ...
    const showAuthenticator = (): ReactElement => {
        if (availableAuthenticators.length > 0) {
            ...
            if (authenticator.id === IdentityProviderManagementConstants.OIDC_AUTHENTICATOR_ID) {
                ...
                if (!authenticator.meta.properties?.find((prop: CommonPluggableComponentMetaPropertyInterface) =>
                    prop.key === "OIDCLogoutEPUrl")) {
