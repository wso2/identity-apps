    ...
    const getExistingAPIResources = (): void => {
        ...
        role?.permissions?.forEach((permission: RolePermissionInterface | string) => {
            ...
            if (typeof permission !== "string") {
                ...
                // Pattern: api-resources/<apiresourceId>/scopes
                const parts: string[] = permission["$ref"].split("/");
                const apiResourcesIndex: number = parts.indexOf("api-resources");
                const apiResourceId: string = apiResourcesIndex !== -1 ? parts[apiResourcesIndex + 1] : null;
        ...
        <Grid container direction="column" justifyContent="center" alignItems="flex-start" spacing={ 2 }>
            <Grid xs={ 8 }>
                <Autocomplete
                    ...
                    noOptionsText={ role?.audience?.type?.toUpperCase() === RoleAudienceTypes.APPLICATION ? t("roles:addRoleWizard.forms.rolePermission.apiResource.hint.noApiResourcesAvailable") : t("common:noResultsFound") }
