...
<%
    String isCookieproEnabled = System.getenv("is_cookiepro_enabled");
    if (isCookieproEnabled == null) {
        isCookieproEnabled = "false";
    }
    String cookiepro_domain_script_id = System.getenv("cookiepro_domain_script_id");
    String initialScriptType = (Boolean.TRUE.toString()).equals(isCookieproEnabled) ? "text/plain" : "text/javascript";
%>

<%
    if (!StringUtils.isBlank(cookiePolicyURL) && 
        ((Boolean.FALSE.toString()).equals(isCookieproEnabled) || 
        ((Boolean.TRUE.toString()).equals(isCookieproEnabled) && !StringUtils.equals(tenantForTheming, IdentityManagementEndpointConstants.SUPER_TENANT)))) {
%>
...
<%
    if (!StringUtils.isBlank(cookiePolicyURL) && 
        (Boolean.TRUE.toString()).equals(isCookieproEnabled) && 
